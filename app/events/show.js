import { fetchQuestion, upVoteAnswer, downVoteAnswer } from '../fetch_data/questions.js';
...
export function show(id) {
  ...
  function insertQuestion(response) {
    ...
    var avatar = response.data.question.avatar;

    //INJECT QUESTION INFO

    var showQuestion = 'question-content-'+ questionId;
    showContent = '<div class="card mb-0 mt-2"> <div class="card-body auxilium-wrapper"> <!-- question --> <div class="auxilium-question d-flex flex-row bd-highlight justify-content-between align-content-center"> <div class="mr-5"> <img src="'+ avatar +'" class="rounded-circle" style="max-width:50px;"> </div> <div class="pr-4" style="flex-grow: 1; width: 80%"> <p class="text-secondary font-italic font-weight-light mb-3 mt-0" style="font-size:0.8em;">Posted '+ timeStamp +' by '+ ownerName +'</p> <h2 class="mt-0">'+ questionTitle +'</h2><div id="' + showQuestion + '"> </div> <div class="d-flex flex-row justify-content-between align-content-center p-3"> <h2 class="text-secondary mt-auto">'+ numberOfAnswers +' Answers</h2> <a class="btn btn-default mt-auto" style="max-height:40px;" href"" id="createAnswer">Post an answer</a> </div> </div> </div> <!-- question end --> </div></div>';
    ...
    for (var i = 0; i < numberOfAnswers; i++) {
      ...
      var avatar = response.data.question.answers[i].avatar;
      var upVoteId = "up-id-" + answerId;
      var downVoteId = "down-id-" + answerId;
    ...
    for (var i = 0; i < numberOfAnswers; i++) {
      ...
      if(response.data.question.answers[i].answer_votes){

        var btnUpVote = document.querySelector(upVoteId);

        btnUpVote.addEventListener('click', function() {
            event.preventDefault();
        });

        var btnDownVote = document.querySelector(downVoteId);

        btnDownVote.addEventListener('click', function() {
            event.preventDefault();
        });

        var hasVoted = response.data.question.answers[i].answer_votes.vote;

        if(hasVoted == 1){
          console.log("activate upvote");
        } else if (hasVoted == -1){
          console.log("activate downvote");
        };
      } else{
        ...
        btnUpVote.addEventListener('click', function() {
            event.preventDefault();
            upVoteAnswer(questionId, event.target.parentNode.getAttribute('answer-id'), userName);
        ...
        btnDownVote.addEventListener('click', function() {
            event.preventDefault();
            downVoteAnswer(questionId, event.target.parentNode.getAttribute('answer-id'), userName);
            console.log(questionId);
            show(questionId);
        });
      };
