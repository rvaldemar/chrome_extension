(function () {
  ...
  //fetchQuestion
  function fetchQuestion(questionId, userName) {
    return axios$1.get(domain + '/api/v1/questions/' + questionId + '?username=' + userName);
  }
  function postNewAnswer(questionId, userName, email, avatar, content) {
    return axios$1.post(domain + '/api/v1/questions/' + questionId + '/answers', {
      'answer': {
        "username": userName,
        "email": email,
        "avatar": avatar,
  ...
  function postNewQuestion(title, userName, email, avatar, content, category) {
    console.log('teta');
    console.log(category);
    return axios$1.post(domain + '/api/v1/questions/', {
      'question': {
        'title': title,
        'category': category,
        "username": userName,
  ...
  function fetchSearchedQuestions(string, userName, email, avatar) {
    return axios$1.get(domain + '/api/v1/search/?username=' + userName + '&email=' + email + '&avatar=' + avatar + '&query=' + string);
  }

  function postNewComment(questionId, userName, email, avatar, content, answerId) {
    return axios$1.post(domain + '/api/v1/questions/' + questionId + '/answers/' + answerId + '/comments', {
      'comment': {
        "username": userName,
        "email": email,
        "avatar": avatar,
  ...
  function upVoteAnswer(questionId, answerId, userName) {
    return axios$1.patch(domain + '/api/v1/questions/' + questionId + '/answers/' + answerId + '/upvote', {
      'answer': {
        "username": userName
      }
    });
  ...
  function downVoteAnswer(questionId, answerId, userName) {
    return axios$1.patch(domain + '/api/v1/questions/' + questionId + '/answers/' + answerId + '/downvote', {
      'answer': {
        "username": userName
      }
    });
  ...
  var Parchment = unwrapExports(parchment);
  ...
  class BlockEmbed extends Parchment.Embed {
    ...
    delta() {
      return new delta().insert(this.value(), extend$1(this.formats(), this.attributes.values()));
    }
  ...
  class Block extends Parchment.Block {
    ...
    delta() {
      if (this.cache.delta == null) {
        this.cache.delta = this.descendants(Parchment.Leaf).reduce((delta$$1, leaf) => {
          if (leaf.length() === 0) {
            return delta$$1;
          } else {
            return delta$$1.insert(leaf.value(), bubbleFormats(leaf));
          }
        }, new delta()).insert('\n', bubbleFormats(this));
      }
  ...
  function bubbleFormats(blot, formats = {}) {
    ...
    if (typeof blot.formats === 'function') {
      formats = extend$1(formats, blot.formats());
    }
  ...
  class CodeBlock extends Block {
    ...
    delta() {
      ...
      return text.split('\n').reduce((delta$$1, frag) => {
        return delta$$1.insert(frag).insert('\n', this.formats());
      }, new delta());
    }
  ...
  */
  ...
  class Editor {
    ...
    applyDelta(delta$$1) {
      ...
      delta$$1 = normalizeDelta(delta$$1);
      delta$$1.reduce((index, op$$1) => {
        ...
        if (op$$1.insert != null) {
          if (typeof op$$1.insert === 'string') {
            ...
            let formats = extend$1({}, bubbleFormats(line));
            if (line instanceof Block) {
              let [leaf, ] = line.descendant(Parchment.Leaf, offset);
              formats = extend$1(formats, bubbleFormats(leaf));
            }
      ...
      delta$$1.reduce((index, op$$1) => {
        if (typeof op$$1.delete === 'number') {
          this.scroll.deleteAt(index, op$$1.delete);
          return index;
        }
        return index + (op$$1.retain || op$$1.insert.length || 1);
      }, 0);
      this.scroll.batchEnd();
      return this.update(delta$$1);
    }
    ...
    getDelta() {
      return this.scroll.lines().reduce((delta$$1, line) => {
        return delta$$1.concat(line.delta());
      }, new delta());
    }
    ...
    getFormat(index, length = 0) {
      ...
      return extend$1.apply(extend$1, formatsArr);
    }

    getText(index, length) {
      return this.getContents(index, length).filter(function(op$$1) {
    ...
    update(change, mutations = [], cursorIndex = undefined) {
      ...
      if (mutations.length === 1 &&
        ...
        change = diffDelta.reduce(function(delta$$1, op$$1) {
          if (op$$1.insert) {
            return delta$$1.insert(op$$1.insert, formats);
          } else {
            return delta$$1.push(op$$1);
          }
        }, new delta());
  ...
  function normalizeDelta(delta$$1) {
    return delta$$1.reduce(function(delta$$1, op$$1) {
      if (op$$1.insert === 1) {
        let attributes = clone_1(op$$1.attributes);
        delete attributes['image'];
        return delta$$1.insert({ image: op$$1.attributes.image }, attributes);
      }
      if (op$$1.attributes != null && (op$$1.attributes.list === true || op$$1.attributes.bullet === true)) {
        op$$1 = clone_1(op$$1);
        if (op$$1.attributes.list) {
          op$$1.attributes.list = 'ordered';
        } else {
          op$$1.attributes.list = 'bullet';
          delete op$$1.attributes.bullet;
        }
      }
      if (typeof op$$1.insert === 'string') {
        let text = op$$1.insert.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
        return delta$$1.insert(text, op$$1.attributes);
      }
      return delta$$1.push(op$$1);
    }, new delta());
  }
  ...
  let debug$1 = namespace('quill:events');
  ...
  class Emitter extends eventemitter3 {
    constructor() {
      super();
      this.listeners = {};
      this.on('error', debug$1.error);
    }

    emit() {
      debug$1.log.apply(debug$1, arguments);
  ...
  class Quill {
    ...
    constructor(container, options = {}) {
      ...
      if (this.container == null) {
        return debug$3.error('Invalid Quill container', container);
      }
  ...
  function expandConfig(container, userConfig) {
    ...
    let themeConfig = extend$1(true, {}, userConfig.theme.DEFAULTS);
    ...
    userConfig = extend$1(true, {}, Quill.DEFAULTS, { modules: moduleConfig }, themeConfig, userConfig);
  ...
  let DirectionAttribute = new Parchment.Attributor.Attribute('direction', 'dir', config$1);
  let DirectionClass = new Parchment.Attributor.Class('direction', 'ql-direction', config$1);
  let DirectionStyle = new Parchment.Attributor.Style('direction', 'direction', config$1);
  ...
  let FontClass = new Parchment.Attributor.Class('font', 'ql-font', config$2);
  ...
  let FontStyle = new FontStyleAttributor('font', 'font-family', config$2);
  ...
  let debug$4 = namespace('quill:clipboard');
  ...
  class Clipboard extends Module {
    ...
    convert(html) {
      ...
      let delta$$1 = traverse(this.container, elementMatchers, textMatchers);
      // Remove trailing newline
      if (deltaEndsWith(delta$$1, '\n') && delta$$1.ops[delta$$1.ops.length - 1].attributes == null) {
        delta$$1 = delta$$1.compose(new delta().retain(delta$$1.length() - 1).delete(1));
      }
      debug$4.log('convert', this.container.innerHTML, delta$$1);
      this.container.innerHTML = '';
      return delta$$1;
    }
    ...
    onPaste(e) {
      ...
      setTimeout(() => {
        delta$$1 = delta$$1.concat(this.convert()).delete(range.length);
        this.quill.updateContents(delta$$1, Quill.sources.USER);
        // range.length contributes to delta.length()
        this.quill.setSelection(delta$$1.length() - range.length, Quill.sources.SILENT);
  ...
  function applyFormat(delta$$1, format, value) {
    if (typeof format === 'object') {
      return Object.keys(format).reduce(function(delta$$1, key) {
        return applyFormat(delta$$1, key, format[key]);
      }, delta$$1);
    } else {
      return delta$$1.reduce(function(delta$$1, op) {
        if (op.attributes && op.attributes[format]) {
          return delta$$1.push(op);
        } else {
          return delta$$1.insert(op.insert, extend$1({}, {[format]: value}, op.attributes));
        }
      }, new delta());
    }
  }
  ...
  function deltaEndsWith(delta$$1, text) {
    let endText = "";
    for (let i = delta$$1.ops.length - 1; i >= 0 && endText.length < text.length; --i) {
      let op  = delta$$1.ops[i];
  ...
  function traverse(node, elementMatchers, textMatchers) {  // Post-order
    if (node.nodeType === node.TEXT_NODE) {
      return textMatchers.reduce(function(delta$$1, matcher) {
        return matcher(node, delta$$1);
      }, new delta());
    } else if (node.nodeType === node.ELEMENT_NODE) {
      return [].reduce.call(node.childNodes || [], (delta$$1, childNode) => {
        ...
        return delta$$1.concat(childrenDelta);
      }, new delta());
    } else {
      return new delta();
    }
  ...
  function matchAlias(format, node, delta$$1) {
    return applyFormat(delta$$1, format, true);
  }

  function matchAttributor(node, delta$$1) {
    ...
    if (Object.keys(formats).length > 0) {
      delta$$1 = applyFormat(delta$$1, formats);
    }
    return delta$$1;
  }

  function matchBlot(node, delta$$1) {
    let match = Parchment.query(node);
    if (match == null) return delta$$1;
    if (match.prototype instanceof Parchment.Embed) {
      let embed = {};
      let value = match.value(node);
      if (value != null) {
        embed[match.blotName] = value;
        delta$$1 = new delta().insert(embed, match.formats(node));
      }
    } else if (typeof match.formats === 'function') {
      delta$$1 = applyFormat(delta$$1, match.blotName, match.formats(node));
    }
    return delta$$1;
  }

  function matchBreak(node, delta$$1) {
    if (!deltaEndsWith(delta$$1, '\n')) {
      delta$$1.insert('\n');
    }
    return delta$$1;
  ...
  function matchIndent(node, delta$$1) {
    let match = Parchment.query(node);
    if (match == null || match.blotName !== 'list-item' || !deltaEndsWith(delta$$1, '\n')) {
      return delta$$1;
    }
    ...
    if (indent <= 0) return delta$$1;
    return delta$$1.compose(new delta().retain(delta$$1.length() - 1).retain(1, { indent: indent}));
  }

  function matchNewline(node, delta$$1) {
    if (!deltaEndsWith(delta$$1, '\n')) {
      if (isLine$1(node) || (delta$$1.length() > 0 && node.nextSibling && isLine$1(node.nextSibling))) {
        delta$$1.insert('\n');
      }
    }
    return delta$$1;
  ...
  function matchSpacing(node, delta$$1) {
    if (isLine$1(node) && node.nextElementSibling != null && !deltaEndsWith(delta$$1, '\n\n')) {
  ...
  function matchStyles(node, delta$$1) {
    let formats = {};
    ...
    if (parseFloat(style.textIndent || 0) > 0) {  // Could be 0.5in
      delta$$1 = new delta().insert('\t').concat(delta$$1);
    }
    return delta$$1;
  }

  function matchText(node, delta$$1) {
    let text = node.data;
    // Word represents empty line with <o:p>&nbsp;</o:p>
    if (node.parentNode.tagName === 'O:P') {
      return delta$$1.insert(text.trim());
    }
    ...
    if (!computeStyle(node.parentNode).whiteSpace.startsWith('pre')) {
      ...
      if ((node.previousSibling == null && isLine$1(node.parentNode)) ||
          (node.previousSibling != null && isLine$1(node.previousSibling))) {
        text = text.replace(/^\s+/, replacer.bind(replacer, false));
      }
      if ((node.nextSibling == null && isLine$1(node.parentNode)) ||
          (node.nextSibling != null && isLine$1(node.nextSibling))) {
        text = text.replace(/\s+$/, replacer.bind(replacer, false));
      }
    }
    return delta$$1.insert(text);
  }
  ...
  let debug$5 = namespace('quill:keyboard');
  ...
  class Keyboard extends Module {
    ...
    addBinding(key, context = {}, handler = {}) {
      ...
      if (binding == null || binding.key == null) {
        return debug$5.warn('Attempted to add invalid keyboard binding', binding);
      }
      ...
      binding = extend$1(binding, context, handler);
  ...
  Keyboard.DEFAULTS = {
    bindings: {
      ...
      'checklist enter': {
        ...
        handler: function(range) {
          let [line, offset] = this.quill.getLine(range.index);
          let formats = extend$1({}, line.formats(), { list: 'checked' });
          let delta$$1 = new delta().retain(range.index)
                                 .insert('\n', formats)
  ...
  let debug$6 = namespace('quill:toolbar');
  ...
  class Toolbar extends Module {
    constructor(quill, options) {
      ...
      if (!(this.container instanceof HTMLElement)) {
        return debug$6.error('Container required for toolbar', this.options);
      }
    ...
    attach(input) {
      ...
      if (this.handlers[format] == null) {
        if (this.quill.scroll.whitelist != null && this.quill.scroll.whitelist[format] == null) {
          debug$6.warn('ignoring attaching to disabled format', format, input);
          return;
        }
        if (Parchment.query(format) == null) {
          debug$6.warn('ignoring attaching to nonexistent format', format, input);
          return;
        }
      }
  ...
  var DropdownIcon = `<svg viewbox="0 0 18 18">
  ...
  BaseTheme.DEFAULTS = extend$1(true, {}, Theme.DEFAULTS, {
    modules: {
      toolbar: {
        handlers: {
          formula: function() {
  ...
  var alignLeft = `<svg viewbox="0 0 18 18">
  ...
  var alignCenter = `<svg viewbox="0 0 18 18">
  ...
  var alignRight = `<svg viewbox="0 0 18 18">
  <line class="ql-stroke" x1="15" x2="3" y1="9" y2="9"></line>
  ...
  var alignJustify = `<svg viewbox="0 0 18 18">
  <line class="ql-stroke" x1="15" x2="3" y1="9" y2="9"></line>
  ...
  var background = `<svg viewbox="0 0 18 18">
  ...
  var blockquote = `<svg viewbox="0 0 18 18">
  ...
  var blockquote$1 = /*#__PURE__*/Object.freeze({
    default: blockquote
  });

  var bold = `<svg viewbox="0 0 18 18">
  ...
  var clean = `<svg class="" viewbox="0 0 18 18">
  ...
  var color = `<svg viewbox="0 0 18 18">
  ...
  var directionLtr = `<svg viewbox="0 0 18 18">
  ...
  var directionLtr$1 = /*#__PURE__*/Object.freeze({
    default: directionLtr
  });

  var directionRtl = `<svg viewbox="0 0 18 18">
  ...
  var directionRtl$1 = /*#__PURE__*/Object.freeze({
    default: directionRtl
  });

  var floatCenter = `<svg viewbox="0 0 18 18">
  ...
  var floatCenter$1 = /*#__PURE__*/Object.freeze({
    default: floatCenter
  });

  var floatFull = `<svg viewbox="0 0 18 18">
  ...
  var floatFull$1 = /*#__PURE__*/Object.freeze({
    default: floatFull
  });

  var floatLeft = `<svg viewbox="0 0 18 18">
  ...
  var floatLeft$1 = /*#__PURE__*/Object.freeze({
    default: floatLeft
  });

  var floatRight = `<svg viewbox="0 0 18 18">
  ...
  var floatRight$1 = /*#__PURE__*/Object.freeze({
    default: floatRight
  });

  var formula = `<svg viewbox="0 0 18 18">
  ...
  var formula$1 = /*#__PURE__*/Object.freeze({
    default: formula
  });

  var header = `<svg viewBox="0 0 18 18">
  ...
  var header2 = `<svg viewBox="0 0 18 18">
  ...
  var header2$1 = /*#__PURE__*/Object.freeze({
    default: header2
  });

  var italic = `<svg viewbox="0 0 18 18">
  ...
  var image = `<svg viewbox="0 0 18 18">
  ...
  var indent = `<svg viewbox="0 0 18 18">
  ...
  var outdent = `<svg viewbox="0 0 18 18">
  <line class="ql-stroke" x1="3" x2="15" y1="14" y2="14"></line>
  <line class="ql-stroke" x1="3" x2="15" y1="4" y2="4"></line>
  <line class="ql-stroke" x1="9" x2="15" y1="9" y2="9"></line>
  ...
  var link = `<svg viewbox="0 0 18 18">
  ...
  var listOrdered = `<svg viewbox="0 0 18 18">
  ...
  var listOrdered$1 = /*#__PURE__*/Object.freeze({
    default: listOrdered
  });

  var listBullet = `<svg viewbox="0 0 18 18">
  ...
  var listBullet$1 = /*#__PURE__*/Object.freeze({
    default: listBullet
  });

  var listCheck = `<svg class="" viewbox="0 0 18 18">
  ...
  var listCheck$1 = /*#__PURE__*/Object.freeze({
    default: listCheck
  });

  var subscript = `<svg viewbox="0 0 18 18">
  ...
  var subscript$1 = /*#__PURE__*/Object.freeze({
    default: subscript
  });

  var superscript = `<svg viewbox="0 0 18 18">
  ...
  var superscript$1 = /*#__PURE__*/Object.freeze({
    default: superscript
  });

  var strike = `<svg viewbox="0 0 18 18">
  ...
  var strike$1 = /*#__PURE__*/Object.freeze({
    default: strike
  });

  var underline = `<svg viewbox="0 0 18 18">
  ...
  var video = `<svg viewbox="0 0 18 18">
  ...
  var require$$0 = ( alignLeft$1 && alignLeft ) || alignLeft$1;

  var require$$1 = ( alignCenter$1 && alignCenter ) || alignCenter$1;

  var require$$2 = ( alignRight$1 && alignRight ) || alignRight$1;

  var require$$3 = ( alignJustify$1 && alignJustify ) || alignJustify$1;

  var require$$4 = ( background$1 && background ) || background$1;

  var require$$5 = ( blockquote$1 && blockquote ) || blockquote$1;

  var require$$6 = ( bold$1 && bold ) || bold$1;

  var require$$7 = ( clean$1 && clean ) || clean$1;

  var require$$8 = ( code$1 && code ) || code$1;

  var require$$9 = ( color$1 && color ) || color$1;

  var require$$10 = ( directionLtr$1 && directionLtr ) || directionLtr$1;

  var require$$11 = ( directionRtl$1 && directionRtl ) || directionRtl$1;

  var require$$12 = ( floatCenter$1 && floatCenter ) || floatCenter$1;

  var require$$13 = ( floatFull$1 && floatFull ) || floatFull$1;

  var require$$14 = ( floatLeft$1 && floatLeft ) || floatLeft$1;

  var require$$15 = ( floatRight$1 && floatRight ) || floatRight$1;

  var require$$16 = ( formula$1 && formula ) || formula$1;

  var require$$17 = ( header$1 && header ) || header$1;

  var require$$18 = ( header2$1 && header2 ) || header2$1;

  var require$$19 = ( italic$1 && italic ) || italic$1;

  var require$$20 = ( image$1 && image ) || image$1;

  var require$$21 = ( indent$1 && indent ) || indent$1;

  var require$$22 = ( outdent$1 && outdent ) || outdent$1;

  var require$$23 = ( link$1 && link ) || link$1;

  var require$$24 = ( listOrdered$1 && listOrdered ) || listOrdered$1;

  var require$$25 = ( listBullet$1 && listBullet ) || listBullet$1;

  var require$$26 = ( listCheck$1 && listCheck ) || listCheck$1;

  var require$$27 = ( subscript$1 && subscript ) || subscript$1;

  var require$$28 = ( superscript$1 && superscript ) || superscript$1;

  var require$$29 = ( strike$1 && strike ) || strike$1;

  var require$$30 = ( underline$1 && underline ) || underline$1;

  var require$$31 = ( video$1 && video ) || video$1;
  ...
  var icons = {
    ...
    'background': require$$4,
    'blockquote': require$$5,
    'bold'      : require$$6,
    'clean'     : require$$7,
    'code'      : require$$8,
    'code-block': require$$8,
    'color'     : require$$9,
    'direction' : {
      ''        : require$$10,
      'rtl'     : require$$11
  ...
  SnowTheme.DEFAULTS = extend$1(true, {}, BaseTheme.DEFAULTS, {
    modules: {
      toolbar: {
        handlers: {
          link: function(value) {
  ...
  function newAnswerOnClick(questionId) {
    ...
    createAnswerBtn.addEventListener('click', function () {
      ...
      submitBtn.addEventListener('click', function () {
        submitNewAnswer(questionId, editor);
      });
    });
  }
  function newQuestionOnClick(questionId) {
    var head = document.querySelector('head');
    ...
    createQuestionBtn.addEventListener('click', function () {
      ...
      var submitBtn = document.getElementById('btn-new-post-submit');
      submitBtn.addEventListener('click', function () {
        submitNewQuestion(editor);
      });
    });
  }
  function newCommentOnClick(questionId, answerId) {
    var createAnswerBtn = document.getElementById('createAnswer');
    ...
    var submitBtn = document.getElementById('btn-new-post-submit');
    submitBtn.addEventListener('click', function () {
      submitNewComment(questionId, editor, answerId);
    });
  }
  ...
  var config$3 = { attributes: true, childList: true, subtree: true };

  observer.observe(targetNode, config$3);
